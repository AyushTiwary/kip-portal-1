...
object ErrorResponses extends JsonHelper {
  ...
  lazy val INVALID_CREDENTIALS = write(ResponseCart(message = Some("Invalid UserName Or Password")))
  lazy val USER_ALREADY_EXISTS = write(ResponseCart(message = Some("User With this email already exists")))
